(** * Common Definitions and Lemmas for Floating-Point Accuracy Proofs
    ...
    This file provides foundational definitions and lemmas used throughout
    the LAProof library. It establishes the core vocabulary
    for reasoning about floating-point rounding, error bounds, and
    accumulation of rounding errors in numerical computations.
    ...
    - _Floating-point rounding_: The function [rounded] captures
      ...
      [fprec t] and exponent range [femax t].

    - _Special floating-point values_: << neg_zero >> and << pos_zero >>
      represent the IEEE 754 signed zero values for a given type << t >>,
      useful when reasoning about sign-sensitive floating-point operations.

    - _Zero testing_: << iszero >> is a boolean predicate on floating-point
      values that returns << true >> exactly when the value is an IEEE 754
      zero (of either sign). The lemma [iszeroR_iszeroF] connects this
      structural test to the real-number interpretation [FT2R x = 0].

    - _Counting nonzeros_: << nnzF >> (resp. << nnzR >>) counts the number of
      nonzero elements in a list of floating-point (resp. real) values.
      Several lemmas formalize the behavior in the case when the nonzero 
      count is zero.

    - _Default relative error bound_ [default_rel]:
      
      << default_rel = (1/2) * 2^(-(fprec t) + 1) >>
      
      This is the unit roundoff (machine epsilon) for type << t >>.
      It satisfies [default_rel > 0] and numerous inequalities involving
      [1 + default_rel] and its powers that are needed in error analysis.

    - _Default absolute error bound_ [default_abs]:
      
      << default_abs = (1/2) * 2^(3 - femax t - fprec t) >>
      
      This bounds the absolute error introduced when rounding a subnormal
      result. It satisfies << default_abs <= default_rel >>.

    - _Relative error accumulation factor_ [g n]:
    
      << g n = (1 + default_rel)^n - 1 >>
      
      This bounds the accumulated relative rounding error after << n >>
    ...
    - _Mixed absolute/relative error accumulation factor_ [g1 n1 n2]:
      ...
      This bounds the accumulated absolute rounding error when
      << n1 >> subnormal rounding errors, each of size [default_abs], are
    ...
    the [commonDB] hint database for use with [auto] and [eauto].
*)
...
(** ** Global Definitions and Setup
    ...
    The definitions below are independent of any particular floating-point
    type and are available without opening [Section WithType]. *)
...
Lemma iszeroR_iszeroF : forall x : ftype t,
...
Proof.
  ...
  - assert (IZR (Z.neg m) * bpow Zaux.radix2 e < 0)%Re; [clear | lra].
    rewrite /IZR.
    apply Rmult_neg_pos.
    + move: (IPR_gt_0 m); lra.
    + move: (bpow_gt_0 Zaux.radix2 e); lra.
  - assert (0 < IZR (Z.pos m) * bpow Zaux.radix2 e)%Re; [clear | lra].
    rewrite /IZR.
    apply Rmult_pos_pos.
    + apply IPR_gt_0.
    + apply bpow_gt_0.
...
(** ** Error Bound Constants
    ...
    Fundamental error parameters for a floating-point type << t >>.
    All ordering and positivity lemmas are collected in [commonDB]. *)
...
Proof.
  apply Rmult_lt_0_compat; try nra; auto with commonDB.
Qed.
...
Proof.
  suff: 1 <= 1 + default_rel; try nra; auto with commonDB.
Qed.
...
Proof.
  eapply Rlt_trans with 1; [nra |]; auto with commonDB.
Qed.
...
Proof.
  ...
  apply: bpow_le => //.
  pose proof fprec_gt_one t; pose proof fprec_lt_femax t; lia.
Qed.
...
Proof. 
  pose proof fprec_gt_one t as Hfprec.
  pose proof fprec_lt_femax t as Hlt.
...
Lemma default_abs_le_fmax :
  default_abs <= fmax.
Proof.
  replace fmax with (1 * fmax) by nra.
  unfold default_abs, fmax.
  apply Rmult_le_compat; try nra.
  - apply bpow_ge_0.
  - apply bpow_le.
    ...
    eapply Z.le_trans with (fprec t + fprec t + femax t)%Z;
      [ | repeat apply Zplus_le_compat_r;
          apply Z.lt_le_incl;
          apply (fprec_lt_femax t) 
          ]. 
    eapply Z.le_trans with (fprec t + fprec t + fprec t)%Z;
      [ | repeat apply Zplus_le_compat_l;
          apply Z.lt_le_incl;
...
Lemma default_abs_ub :
  default_abs <= 1.
Proof.
  pose proof (abs_le_rel) as H.
  eapply Rle_trans; [apply H |].
  unfold default_rel. rewrite  bpow_plus bpow_opp.
  replace (bpow _ 1) with 2 by (simpl; nra).
  refine (Rle_trans _ (1 / bpow Zaux.radix2 (fprec t)) _ _ _).
  - nra.
  - apply Rdiv_le_left.
    + apply bpow_gt_0.
    + refine (Rle_trans _ 2 _ _ _); try nra.
      rewrite Rmult_1_l; apply bpow_fprec_lb_2.
Qed.
...
Proof.
  unfold default_rel.
  pose proof bpow_gt_0 Zaux.radix2 (fprec t) as Hpos.
  rewrite !bpow_plus.
  rewrite <- !Rmult_assoc.
  rewrite Rmult_comm.
  rewrite <- !Rmult_assoc.
...
(** ** Error Accumulation Factors
    ...
    [g n] and [g1 n1 n2] bound accumulated rounding errors over sequences
    of floating-point operations. The [commonDB] hint database is populated
    with the lemmas below to support automated error bound proofs. *)
...
Lemma g_pos n : 0 <= g n.
Proof.
  unfold g; induction n; simpl; try nra.
  eapply Rle_trans; [apply IHn | apply Rplus_le_compat; try nra].
  eapply Rle_trans with (1 * (1 + D) ^ n); try nra.
  apply Rmult_le_compat; try nra.
...
Proof.
  ...
  rewrite Rplus_comm.
  replace (1 + D) with ((1 + D) ^ 1) at 1 by nra.
  apply Rle_pow; [| lia].
  auto with commonDB.
Qed.
...
Proof.
  unfold g; rewrite Rmult_minus_distr_l; rewrite tech_pow_Rmult.
...
Proof.
  ...
  - rewrite <- tech_pow_Rmult; field_simplify; nra.
...
Proof.
  intros.
  unfold g1, g; field_simplify.
  symmetry; rewrite <- tech_pow_Rmult; field_simplify; nra.
Qed.
Hint Resolve g1_pos : commonDB.
...
Proof.
  intros; unfold g1.
  eapply Rle_trans with (1 * E * 1); try nra.
...
Proof.
  ...
  - apply: Rmult_le_compat.
    ...
    + rewrite /g; field_simplify; apply pow_le; auto with commonDB.
    + apply: Rmult_le_compat; try nra; auto with commonDB.
...
Proof.
  ...
  - apply: Rmult_le_compat.
    ...
    + apply: Rmult_le_compat; try nra; auto with commonDB.
    + apply: Rplus_le_compat_l; auto with commonDB.
      rewrite addnC; auto with commonDB.
  - rewrite plus_INR; simpl; nra.
Qed.
Hint Resolve g1n_le_g1Sn' : commonDB.
...
Proof.
  ...
  - apply: Rmult_le_lt_compat; try nra; auto with commonDB.
    ...
    + suff : INR n < INR n + 1; simpl; try nra.
      move => H; rewrite plus_INR; simpl; nra.
...
(** ** Floating-Point Operation Lemmas
    ...
    Structural identities: rounding is idempotent on exact FP values
    ([round_FT2R]); signed-zero behavior of [BPLUS] and [BMINUS];
    commutativity of [BPLUS]; and the equivalence [BMINUS x y = BPLUS x (BOPP y)]. *)
...
Lemma foldl_congr :
  forall (op : ftype t -> ftype t -> ftype t)
...
Proof.
  intros.
  revert u v H bl H0; induction al; destruct bl; simpl; intros;
...
(** ** Automation
    ...
    [field_simplify_Rabs] reduces a goal of the form [Rabs e <= _]
    by simplifying the expression [e] and splitting denominator non-zero
    side conditions. *)
